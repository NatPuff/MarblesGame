...
class GameScene: SKScene {
    ...
    var marbles = ["marbleBlue", "marbleGreen", "marblePurple", "marbleRed", "marbleYellow", "marbleGrey"]
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if matchedMarbles.count >= 3 {
            ...
            for marble in matchedMarbles{
                if let particles = SKEmitterNode(fileNamed: "waterblast") {
                    particles.position = marble.position
                    addChild(particles)
                    
                    let removedAfterMatched =
                    SKAction.sequence([SKAction.wait(forDuration: 3),
                                       SKAction.removeFromParent()])
                    particles.run(removedAfterMatched)
                }
